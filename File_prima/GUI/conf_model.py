...
def label_with_icon(name: str) -> str:
    ...
    return f"{LLM_ADAPTER_ICONS.get(name, '🧩')} {name}"
    ...
    for backend, tab in zip(backends, tabs):
        with tab:
            ...
            if backend == "LM Studio":
                with c1:
                    host = st.text_input(f"{UI_ICONS['Host']} {get_text('conf_model', 'host_lm_studio')}",
                                         value=cfg.get("host", "http://localhost:1234"),
                                         key=k(f"lm_host_{backend}"))
                with c2:
                    flt = st.text_input(f"{UI_ICONS['Filtro']} {get_text('conf_model', 'filter_contains')}", value=cfg.get("filter", ""),
                                        key=k(f"lm_filter_{backend}"))
                cfg.update(host=host, filter=flt)
                lmstudio_panel(host=host, key=f'{backend}_local')
            elif backend == "Ollama":
                with c1:
                    host = st.text_input(f"{UI_ICONS['Host']} {get_text('conf_model', 'host_ollama')}",
                ...
                with c1:
                    tok = st.text_input(f"{UI_ICONS['Hugging Face Token']} {get_text('conf_model', 'hf_token_opt')}",
            ...
            with a1:
                do_load = st.button(f"{UI_ICONS['Load List']} {get_text('conf_model', 'load_list')}", key=k(f"load_{backend}"))
            with a2:
                do_clear = st.button(f"{UI_ICONS['Clear List']} {get_text('conf_model', 'clear_list')}", key=k(f"clear_{backend}"))
            with a3:
                do_reset_btn = st.button(f"{UI_ICONS['Global Reset']} {get_text('conf_model', 'global_reset')}", key=k(f"reset_{backend}"))
            ...
            if models:
                ...
                if backend in ["LM Studio", "Ollama"]:
                    t1, t2, t3 = st.tabs([f"{UI_ICONS['Details']} {get_text('conf_model', 'details')}", f"🧪 {get_text('conf_model', 'test_generation')}", f"🖥️ {get_text('conf_model', 'server_cli')}"])
                elif backend in ("Hugging Face"):
                    t1, t2 = st.tabs([f"{UI_ICONS['Details']} {get_text('conf_model', 'details')}", f"🧪 {get_text('conf_model', 'test_generation')}"])
                else:
                    t1, = st.tabs([f"{UI_ICONS['Details']} {get_text('conf_model', 'details')}"])
                ...
                with t1:
                    ...
                    if isinstance(details, dict):
                        ...
                        if not expl_df.empty:
                            st.markdown(f"### {UI_ICONS['Details']} {get_text('conf_model', 'explanation_details')}")
                        ...
                        # # ---- 3️⃣ RAW JSON ----
                        # with st.expander(f"{UI_ICONS['JSON']} {get_text('conf_model', 'raw_json')}"):
                        #     if 'dataframe' in details:
                        #         details.pop("dataframe")
                        #     if 'explanations_df' in details:
                        #         details.pop("explanations_df")
                        #     if 'Spiegazioni' in details:
                        #         details.pop("Spiegazioni")
                        #     st.json(details)
                ...
                if backend in ("Hugging Face", "LM Studio"):
                    with (t2 if backend != "LM Studio" else t2):
                        ...
                        if st.button(f"{UI_ICONS['Run']} {get_text('conf_model', 'run')}", key=k(f"run_{backend}")):
                        ...
                        if st.session_state.get(out_key) is not None:
                            with st.expander(f"{UI_ICONS['Response']} {get_text('conf_model', 'response')}", expanded=True):
                                st.write(st.session_state[out_key])
                                if st.button(f"{UI_ICONS['Clear Output']} {get_text('conf_model', 'clear_output')}",
...
def configure_online_model(key_prefix):
    ...
    backend_display_options.remove('Local (Upload)')
    ...
    with tab1:
        ...
        if not results and submitted is None:
            ...
            selected = st.selectbox(f"{UI_ICONS['Select a Model']} {get_text('conf_model', 'select_model')}", names, key="hf_model_select")
            ...
            if m:
                ...
                load_model = st.button(f"{UI_ICONS['Load Model']} {get_text('conf_model', 'load_model_btn')}", key="load_model_{}".format(selected))
...
def upload_llm():
    backend_display_options = list(llm_adapters.LLM_ADAPTERS.keys())
    llm_backend = st.selectbox(f"{UI_ICONS['LLM Model Source']} {get_text('conf_model', 'model_source')}", options=backend_display_options,
                               key='llm_backend',
    ...
    if llm_backend:
        ...
        if isinstance(available_models, list) and available_models:
            ...
            if st.session_state.llm_model:
                with st.expander(f"{UI_ICONS['Details']} {get_text('conf_model', 'model_details')}", expanded=True):
...
def spacy_show_model(name):
    ...
    with st.expander(f"{UI_ICONS['Installed Models']} {get_text('conf_model', 'installed_spacy')}", expanded=False):
        ...
        # st.divider()
        st.subheader(f"{UI_ICONS['Install Model']} {get_text('conf_model', 'install_spacy')}")

        col1, col2 = st.columns([3, 1])
        with col1:
            default_idx = 0 if "it_core_news_sm" in COMMON_SPACY_MODELS else 0
            model_to_install = st.selectbox(
                f"{UI_ICONS['Choose a Model']} {get_text('conf_model', 'choose_install')}",
                COMMON_SPACY_MODELS,
                index=default_idx,
        ...
        st.markdown(f"### {UI_ICONS['Selected Model Details']} {get_text('conf_model', 'selected_details')}")
        ...
        if st.button(f"{UI_ICONS['Download Models']} {get_text('conf_model', 'download_model_btn')}", type="primary", disabled=disabled):
...
def show_tab_local_upload(key_prefix, name):
    ...
    def detect_fmt(files: list[str]) -> str:
        ...
        if "config.json" in lower and (any(f.endswith(".safetensors") for f in lower)
                                       or any(f.endswith("pytorch_model.bin") for f in lower)):
            # tokenizer: ok se esiste almeno uno tra tokenizer.json / tokenizer.model / vocab.json
            if any(x in lower for x in ("tokenizer.json", "tokenizer.model", "vocab.json")):
    ...
    model_name = st.text_input(f"{UI_ICONS['Metadata']} {get_text('conf_model', 'model_name_folder')}",
    ...
    if st.button(f"{UI_ICONS['Save']} {get_text('conf_model', 'save_local_btn')}", key=k("save_local_model")):
        if not files:
    ...
    if not models:
        ...
        sel = st.selectbox(f"{UI_ICONS['Select a Model']} " + get_text("conf_model", "select_model"), options=models, key=k("local_pick"))
...
def hugging_face_tab():
    ...
    with st.form(key="formHF"):
        ...
        use_token = st.text_input(f"{UI_ICONS['Hugging Face Token']} " + get_text("conf_model", "hf_token_help"),
                                  type="password", value=get_HF_Token())
        submit = st.form_submit_button(f"{UI_ICONS['Filtro']} " + get_text("conf_model", "search_btn"))
...
def lmstudio_tab():
    ...
    col1, col2 = st.columns(2)
    with col1:
        lm_host = st.text_input(f"{UI_ICONS['Host']} " + get_text("conf_model", "host_lm_studio"), "http://localhost:1234",
                                help=get_text("conf_model", "lm_host_help"))
    with col2:
        lm_filter = st.text_input(f"{UI_ICONS['Filtro']} " + get_text("conf_model", "filter_contains"), "", placeholder="es. qwen, mistral, llama")
    ...
    if st.session_state['server_lmStudio'] is True:
        ...
        with cols_srv[0]:
            if st.button(f"{UI_ICONS['Refresh']} " + get_text("conf_model", "refresh_cache"), help=get_text("conf_model", "refresh_cache_help")):
                st.cache_data.clear()
        ...
        st.subheader(f"{UI_ICONS['Download Models']} " + get_text("conf_model", "download_lms_hub"))
        ...
        with st.form(key="lms_get_form"):
            ...
            submit_get = st.form_submit_button(f"{UI_ICONS['Download Models']} " + get_text("conf_model", "download_with_lms"))
...
def ollama_tab():
    ...
    with colh1:
        ollama_host = st.text_input(f"{UI_ICONS['Host']} " + get_text("conf_model", "host_ollama"), "http://localhost:11434",
                                    help=get_text("conf_model", "ollama_host_help"))
    with colh2:
        reg_query = st.text_input(f"{UI_ICONS['Filtro']} " + get_text("conf_model", "repo_filter"), "",
    ...
    if st.session_state['server_ollama'] is True:
        ...
        with st.expander(f"{UI_ICONS['Filtro']} " + get_text("conf_model", "search_registry"), expanded=True):
            c1, c2 = st.columns([2, 1])
            with c1:
                reg_filter = st.text_input(f"{UI_ICONS['Filtro']} " + get_text("conf_model", "search_repo_contains"), reg_query or "",
                                           placeholder="es. llama3, mistral, qwen")
            with c2:
                refresh_reg = st.button(f"{UI_ICONS['Refresh']} " + get_text("conf_model", "refresh_registry"))
            ...
            if repos:
                repo = st.selectbox(f"{UI_ICONS['Select a Model']} " + get_text("conf_model", "select_repo"), repos, index=0,
                                    key="oll_reg_repo")
...
def spacy_tab():
    ...
    with colh1:
        filtro = st.text_input(f"{UI_ICONS['Filtro']} " + get_text("conf_model", "filter_contains"), "",
...
# ========== DETTAGLI (anche senza download) ==========
    ...
    with dl_cols[0]:
        do_dl = st.button(f"{UI_ICONS['Download Models']} " + get_text("conf_model", "download_update"), key="spacy_dl_go")
    with dl_cols[1]:
